...
class ScryptTests: XCTestCase {
    func testScrypt() {
        ...
        let deriver = try! Scrypt(password: password, salt: salt, dkLen: 64, N: 1024, r: 8, p: 16)
    ...
    func testLibsodiumPerformance() {
        ...
        let salt = Data.random(length: 32)
        measure {
            _ = scrypt(password: password, salt: salt, length: 32, N: 4096, R: 6, P: 1)!.bytes
        }
    }
