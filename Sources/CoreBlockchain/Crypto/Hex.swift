...
extension String {
    ...
    public func hexToData() throws -> Data {
        ...
        while cIndex < data.count {
            if index + 1 < string.count {
                ...
                guard a != 1 else { throw HexError.invalidHexFormat }
                let b = leftMap[Int(string[index+1])]
                guard b != 16 else { throw HexError.invalidHexFormat }
                output[cIndex] = a | b
                index += 2
                cIndex += 1
            } else {
                let a = leftMap[Int(string[index])]
                guard a != 16 else { throw HexError.invalidHexFormat }
                output[cIndex] = a
                return data
            }
        }
