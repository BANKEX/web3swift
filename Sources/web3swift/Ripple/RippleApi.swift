...
extension URL {
    static var rippleMainnet = URL(string: "https://s2.ripple.com:51234")!
    static var rippleTestnet = URL(string: "https://s.altnet.rippletest.net:51234")!
}
...
class RippleApi {
    ...
    init(network: NetworkProvider) {
        ...
        account = RippleAccountsApi(network: network)
    }
    static var mainnet: RippleApi {
        return RippleApi(network: NetworkProvider(url: .rippleMainnet))
    }
    static var testnet: RippleApi {
        return RippleApi(network: NetworkProvider(url: .rippleTestnet))
    }
    
    let account: RippleAccountsApi
}
