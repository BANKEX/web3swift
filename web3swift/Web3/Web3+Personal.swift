...
extension web3.Personal {
    ...
    public func signPersonalMessage(message: Data, from: EthereumAddress, password: String = "BANKEXFOUNDATION") throws -> Data {
        return try self.signPersonalMessagePromise(message: message, from: from, password: password).wait()
    }
     ...
     */
    public func unlockAccount(account: EthereumAddress, password:String = "BANKEXFOUNDATION", seconds: UInt64 = 300) throws -> Bool {
        return try self.unlockAccountPromise(account: account).wait()
    }
