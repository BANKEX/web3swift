...
extension web3 {
    ...
    public func contract(_ abiString: String, at: EthereumAddress? = nil) throws -> Web3Contract {
        return try Web3Contract(web3: self, abiString: abiString, at: at, options: self.options)
    }
    ...
    public class Web3Contract {
        ...
        public init(web3 web3Instance:web3, abiString: String, at: EthereumAddress? = nil, options: Web3Options? = nil) throws {
        ...
        public func deploy(bytecode: Data, parameters: [Any], extraData: Data = Data(), options: Web3Options?) throws -> TransactionIntermediate {
            ...
            var tx = try self.contract.deploy(bytecode: bytecode, parameters: parameters, extraData: extraData, options: mergedOptions)
            tx.chainID = self.web3.provider.network
            return TransactionIntermediate(transaction: tx, web3: self.web3, contract: self.contract, method: "fallback", options: mergedOptions)
        }
        ...
        public func method(_ method: String = "fallback", parameters: [Any], extraData: Data = Data(), options: Web3Options?) throws -> TransactionIntermediate {
            let mergedOptions = self.options.merge(with: options)
            var tx = try self.contract.method(method, parameters: parameters, extraData: extraData, options: mergedOptions)
            tx.chainID = self.web3.provider.network
            return TransactionIntermediate(transaction: tx, web3: self.web3, contract: self.contract, method: method, options: mergedOptions)
        }
